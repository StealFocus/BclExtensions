...
namespace StealFocus.BclExtensions
{
    ...
    public static class Resource
    {
        ...
        /// None.
        /// </remarks>
        /// <exception cref="BclExtensionsException">Thrown when the <see cref="ResourceManager"/> returns a null resource string for the provided key.</exception>
        public static string GetString(Type requesterType, string key)
        {
            ...
            {
                throw new BclExtensionsException(string.Format(CultureInfo.CurrentCulture, "No Resource String matching the key '{0}' could be found for type '{1}'.", key, requesterType.FullName));
            }
        ...
        /// </returns>
        /// <remarks>
        /// The <c>assemblyName</c> should be the assembly file name without the <![CDATA[".dll"]]> extension. The <c>xmlDocumentResourceName</c> should be the fully qualified name of the resource e.g. "Namespace.DocumentName.xml".
        /// </remarks>
        ...
        public static XmlDocument GetXmlDocument(Assembly assembly, string xmlDocumentResourceName)
        {
            ...
            using (Stream resourceStream = assembly.GetManifestResourceStream(xmlDocumentResourceName))
            {
                if (resourceStream == null)
                {
                    ...
                    throw new BclExtensionsException(exceptionMessage);
                }
        ...
        /// </returns>
        /// <remarks>
        /// The <c>assemblyName</c> should be the assembly file name without the <![CDATA[".dll"]]> extension. The <c>xsdDocumentResourceName</c> should be the fully qualified name of the resource e.g. "Namespace.DocumentName.xml".
        /// </remarks>
        ...
        public static XmlSchema GetXmlSchema(Assembly assembly, string xsdDocumentResourceName)
        {
            ...
            using (Stream resourceStream = assembly.GetManifestResourceStream(xsdDocumentResourceName))
            {
                ...
                catch (XmlException e)
                {
                    ...
                    throw new BclExtensionsException(exceptionMessage, e);
                }
        ...
        public static void GetBinaryFileAndWriteToPath(Assembly assembly, string fileResourceName, string filePathToBeWrittenTo)
        {
            ...
            try
            {
                fs = new FileStream(filePathToBeWrittenTo, FileMode.Create);
                bw = new BinaryWriter(fs);
                stream = GetFile(assembly, fileResourceName);
